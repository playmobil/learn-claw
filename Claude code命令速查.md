![image-20260327085703452](./pics/image-20260327085703452.jpg)



### Claude Code 速查表

**最后更新**：2026年3月23日 (Claude Code v2.1.81)
...
- `/theme` 更改颜色主题
- `/permissions` 查看/更新权限
- `/effort [level]` 设置努力度 (low/med/high) `[NEW]`
- `/color [color]` 设置提示栏颜色
...
- `/insights` 分析会话报告
- `/desktop` 在桌面应用中继续
- `/remote-control` 桥接终端到 claude.ai/code `[NEW]`
- `/stickers` 订购贴纸！🎉
...
- `worktree.sparsePaths` 稀疏检出目录 `[NEW]`
...
- `maxTurns` 限制代理轮次
- `SendMessage` 恢复代理 (替代 resume) `[NEW]`
...
- `--verbose` 详细模式
- `--bare` 最小化无头 (无 hooks/LSP) `[NEW]`
- `--channels` 权限中继 / MCP 推送 `[NEW]`
- `--remote` Web 会话
- `--chrome` Chrome
